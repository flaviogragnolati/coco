...
| Whether `operation.rollOver.open` should keep firing on the operation that *owns* a reverted roll over | list noise vs worklist fidelity | keep firing — a reverted roll over genuinely needs a new operation, which is exactly what the rule is for | revisit after Phase 2 runs on real data |
...
| --- | --- | --- |
| A1 | **The end-to-end run**, owed since Phase 1 and never performed for 1, 2, 3, 4a, 4b or 5. Its twelve steps are in §21.7 | No evidence of execution. The development database is **not freshly seeded**: it carries two ad-hoc operations (`OP-1781289532189-…`, `OP-1784998903848-…`), both `completed` with **zero lots** and all demand in `preAllocation` roll overs — leftovers of manual UI testing. Phase 5's `tsx` script (§21.8) remains the only executed service-layer path |
| A2 | **Seed realignment** (`prisma/seed.ts`) | `grep` returns **zero** `leg:` and **zero** `deliveryMode:` occurrences in the seed. The database confirms all three packages at `leg: inbound` — two of them on `endUserDelivery` shipments — and **both** end-user shipments at `deliveryMode: null`. **Correction to §21.7:** that is **two** live `shipment.endUser.noDeliveryMode` criticals, not one; the seed has two `endUserDelivery` shipments (`SHIP-SEED-ENDUSER-001` and `-002`), not one. Full drift tables: §21.2, §21.4, §21.7, §21.8 |
...
| # | Item | Verified state 2026-07-27 |
| --- | --- | --- |
| B1 | **The entire series is uncommitted.** Phases 0–5 — roughly 57 modified and 90 untracked files — live in the working tree. `git log` shows nothing since `5feee79`, which predates Phase 0 | A single `git status` confirms it. Rollback strategy in "Rollout & migration strategy" above is "revert the phase's code", which is not currently possible per phase |
| B2 | **The remediation log stops at Phase 2.** `docs/code-review-codebase-2026-07-22.md` ends at §17 (Phase 2); Phases 3, 4a, 4b and 5 have no entry | §21.4–§21.8 are the as-built record and are complete, so this is a cross-reference gap rather than lost information — but the review document's own convention ("the logs are append-only here") is broken |
...
| --- | --- | --- | --- |
| C1 | The four `after N days` diagnostics: `dispatch.notReceived`, `package.received.notFractionated`, `package.outbound.notCollected`, `carrierOrder.requestedNotConfirmed` | §20.3, Phase 3/4a/4b/5 grills | `operation.rollOver.stale` is the only precedent: a threshold computed once per request in the data layer (`findStaleOpenRollOverThreshold`, `STALE_ROLLOVER_OPERATION_LAG = 2`) and passed into a **pure** rule via an options argument. `calculateOperationDiagnostics` is the only calculator that currently takes options; the other four take none and must gain one |
| C2 | A **"sin orden de transporte"** filter on the shipments page | §20.3, Phase 5 grill | `shipmentListInputSchema.unassigned` **already exists** (added by Phase 5) and is consumed only by `carrier-order-shipment-picker.tsx`. This is a filter-panel control on `shipments-client.tsx`, nothing more. Pairs with the existing `shipment.carrierOrder.missing` rule |
| C3 | **Multi-select fractionation** on the packages list feeding `package.fractionate`'s `sourcePackageIds` | §20.3, §21.6 | The command already takes a list and groups by cart across the whole selection; `package-fractionate-dialog.tsx:121` sends `[pkg?.id ?? 0]`. The dialog is driven off a single `PackageDetail`, so this needs a multi-package candidate source, not just a checkbox column |
| C4 | Delegating `lotAvailableActions` to `supplierOrderAvailableActions` for real enablement | §20.3, §21.7 | `lotAvailableActions({ supplierOrderCode })` returns every supplier-order key uniformly disabled. Real enablement needs `supplierOrderAvailableActions`' four inputs — `status`, `operationCompleted`, `liveLineCount`, `dispatchableQuantity` — computed **order-wide**, across all the order's lots, which the current lot select does not carry |
