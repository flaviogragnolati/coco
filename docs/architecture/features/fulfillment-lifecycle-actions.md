...
**Edge case:** it refuses (`CONFLICT`) while the shipment is itself `delayed`/`failed`. `isDisrupted` reads **both** levels, so recovering only the package would leave the item deriving `exception` anyway — a command that appears to do nothing. Recovering the shipment first is the supported order — **and until 2026-09-18 there was no command to do it** (QA #61): a delayed shipment could only leave through `receive`, `deliver` or `markFailed`. See the next scenario.

### Scenario: Shipment recovery — ✅ **as built, 2026-09-18**
**Actor / trigger:** a delayed shipment is moving again.
**Flow:** `shipment.recover` (`delayed` only) returns the shipment to where the delay caught it, with the target **derived, never asked**. `Shipment` records no departure time, so the target reads the `before.status` of the latest `shipment.markDelayed` audit entry: delayed before departure → `readyForDispatch` (its delayed packages → `readyForShipment`, lines `packed`); otherwise, including no audit trail → `inTransit` (packages → `inTransit`, lines `shipped`). Publishes `fulfillment.exception.resolved` per affected cart item with trigger `recover`. The detail exposes the target as `recoveryTarget` so the dialog states it. `shipmentTransitions.delayed` gained `readyForDispatch` for this.
**Edge case:** once the shipment is recovered its packages are no longer disrupted, so `package.recover` is not needed afterwards. `markDelayed` is the undo. Because recovery makes delay → recover → delay routine, the exception and resolution event keys carry the occurrence after the first (the outbox skips duplicate keys, which would otherwise hide the second incident from the customer).
...
**Flow:** one command, operator-editable parameters (preloaded from the source operation, `includeRollOver` defaulted to `true`, and **forced** to `true` on the `completed` path — revised 2026-09-18, QA #45), three paths by status — `failed` updates the same row and re-executes in place; `completed` compensates as above and then creates and executes a new operation; `cancelled` creates and executes only. All of it in one `Serializable` transaction: if execution throws, the compensation rolls back with it and no `failed` row survives, deliberately unlike `createAndExecute`.
**Edge case:** a `completed` source must satisfy the same administrative window as cancel, since its first step *is* a compensation. A re-run that cloned `includeRollOver: false` would produce an empty operation and strand everything the compensation just released. A default alone did not prevent it (QA #45: the operator switched it off and the server honoured it), so since 2026-09-18 the server overrides the flag on the `completed` path (`rerunParameters`, audited as `forcedIncludeRollOver`) and the dialog shows the switch on and disabled. `failed` and `cancelled` sources released nothing and keep the operator's choice.
...
- **Monitoring = diagnostics**: each phase's correctness is observable on the existing diagnostics surfaces; they gained leg scoping (**shipped Phase 3**: `package.leg.overAllocated`, `package.leg.missingShipment`, plus `shipment.failedWithoutFollowUp`, `shipment.received.linesNotReceived` and two new `supplierOrder.*` rules), cancelled-record scoping (**shipped Phase 1**, plus four new `supplierOrder.*` rules, **extended to the package and shipment rules in Phase 3**), and a "roll over open older than N operations" signal (**shipped Phase 0**). **Phase 4b added the delivery rules** (§21.7): `shipment.endUser.noDeliveryMode` (critical), `shipment.pickupPoint.pendingCollection` (warning), and a severity refinement on `package.outbound.multiCustomer` — warning on a pickup point, critical on a home delivery. It also **exempts a delivered `pickupPoint` shipment from all three `received`-row rules**, because its packages correctly stay `inTransit` until collected; the exemption is scoped to that mode only, and `pendingCollection` replaces the signal rather than removing it. **Phase 5 added its own module** (§21.8) with four `carrierOrder.*` rules, all `warning`: `status.aggregateAheadOfShipments`, `noShipments`, `closedWithLiveShipments` and `shipment.disrupted` — the aggregate rule exempts disrupted shipments and hands them to the disruption rule, which is itself silent on a `cancelled`/`failed` booking. It is `warning` rather than `critical` on purpose: a booking carries no quantity and nothing derives from its status.
  ...
  **The closure added the four `after N days` rules** (§21.10), all `warning`, and the exemption the compensated-operation case needed:
  `shipment.dispatch.notReceived`, `package.received.notFractionated`, `package.outbound.notCollected` and `carrierOrder.requestedNotConfirmed`.
  They follow `operation.rollOver.stale`'s shape exactly — a threshold computed **once per request in the data layer**, passed into a pure rule
  through an options argument, so the rule itself stays a function of its record. The three logistics rules anchor on `updatedAt` at 7 days,
  which is sound only because of what the commands do *afterwards* (`fractionate` never touches its sources, an `inTransit` shipment's remaining
  writes are all terminal); the failure mode is a *late* warning, never a false one. `carrierOrder.requestedNotConfirmed` anchors on the real
  `requestedAt` column at 3 days — a carrier should answer faster than goods move. Do **not** add `receivedAt`/`dispatchedAt` columns to sharpen
  the approximation: that is the modelling decision §15.3 defers, not a fix.

  **Corrected by the closure:** `lot.cancelledWithActiveDemand` and `supplierOrder.cancelledWithActiveDemand` fired `critical` on **every correct
  compensation**, which is status-only and returns its cart items to `awaitingAggregation` — unresolved demand by those rules' definition, and the
  right outcome. Both now exempt a record whose operation is `cancelled`, mirroring the operation-level exemption below. A lot cancelled through
  the *supplier loop* keeps the rule, because that path mints roll overs instead. Diagnostics never mutate.
...
| A re-run clones `includeRollOver: false` and strands what the compensation just released | demand silently parked in open roll overs | medium | high | the server forces the flag to `true` for a `completed` source and the dialog locks it (2026-09-18); `operation.rollOver.stale` catches survivors |
...
- ~~A `dispatch.notReceived after N days` diagnostics code, complementing `operation.rollOver.stale`, its `package.received.notFractionated` sibling, and a `package.outbound.notCollected` sibling for pickup points.~~ **All three shipped in the closure** (§21.10, C1), together with the carrier-side sibling below — four rules, all `warning`, each with the threshold computed once per request in the data layer.
...
- ~~A multi-select on the packages list feeding `package.fractionate`'s `sourcePackageIds`.~~ **Shipped in the closure** (§21.10, C3), though not as a list multi-select: `admin.package.fractionationCandidates` computes the editable rows server-side across the whole selection — reusing the command's own per-demand budget loop, so the dialog cannot offer a total the command would refuse — and an `InboundPackagePicker` adds sibling received sources inside the dialog. The shared `crud-table.tsx` gained no row selection, which would have touched every admin table for one dialog.
- ~~Delegating `lotAvailableActions` to `supplierOrderAvailableActions` for real enablement rather than a uniformly-disabled list.~~ **Shipped in the closure** (§21.10, C4). It delegates when handed the order-wide facts and stays uniformly disabled without them, so the two surfaces can never disagree. The facts are read once per **distinct supplier order** across a page (`findSupplierOrderActionInputs`) rather than by widening `lotSummarySelect`, which would have cost a nested read per lot row. Lots still get no commands of their own (ADR 0003) — only what the buttons say and whether they click through.
- A diagnostics code for "supplier **order** requested more than N days ago without confirmation" (raised during the Phase 1 grill) is **still not built**. Its carrier-side sibling ~~`carrierOrder.requestedNotConfirmed after N days`~~ **shipped in the closure** (§21.10, C1) at 3 days, anchored on the real `requestedAt` column; the supplier-order version would need the same treatment on `SupplierOrder.requestedAt`.
- **Carrier-order refinements raised during the Phase 5 grill and deliberately not built** (§21.8): promoting `carrierOrder.status.aggregateAheadOfShipments` from `warning` to `critical`, *only* if a consumer ever derives from booking status — today nothing does, which is the whole argument for `warning`; a `carrierOrder.deletedWithLiveShipments` rule, currently unreachable by command because `softDelete` refuses while live shipments hang off the order; a `restore` command for soft-deleted bookings, withheld for parity with `carrier`, which has none either; ~~a "sin orden de transporte" filter on the shipments page, pairing with the existing `shipment.carrierOrder.missing` rule~~ (**shipped in the closure**, §21.10, C2); and a structured zod schema for `CarrierOrder.metadata`, as the `TODO`s on `Carrier.address`/`contactInfo` already foresee for that entity.
...
**Seed drift Phase 1 introduces or inherits** (enumerated, not fixed — `prisma/seed.ts` is untouched). **Resolved by the closure, 2026-07-27 (§21.10): every drift below is fixed, and `pnpm db:seed-verify` now fails if any of it returns.**

As enumerated at the time:
...
**Seed drift Phase 2 introduces or inherits** (enumerated, not fixed — `prisma/seed.ts` is untouched). **Resolved by the closure, 2026-07-27 (§21.10): every drift below is fixed, and `pnpm db:seed-verify` now fails if any of it returns.**
...
**Seed drift Phase 3 introduces or inherits** (enumerated, not fixed — `prisma/seed.ts` is untouched). **Resolved by the closure, 2026-07-27 (§21.10): every drift below is fixed, and `pnpm db:seed-verify` now fails if any of it returns.**
...
**Partly verified against the real database — a first for the series.** The suite is still `environment: "node"` with no DB fixture harness, but Phase 5's service layer was driven directly against the seeded Neon database by a throwaway `tsx` script — **method corrected 2026-07-27 (§21.10): that generalizes only with `npx tsx --conditions=react-server`.** Every other command service imports `DomainEventDispatcher`, which imports `server-only`; without the flag the import throws "This module cannot be imported from a Client Component module". `carrier-order.service.ts` publishes nothing and so imports no dispatcher, which is the sole reason this script worked and why nothing else would have — (created a booking, walked `pending → requested → confirmed`, exercised the refusals, cancelled it, soft-deleted it, then restored and purged it; the record and its audit rows were removed and the seeded three were left untouched). What that **executed**, not merely reviewed:
...
**Deliberately still owed — Phase 5 did not close the series.** **All of it except migration baselining was discharged by the closure (§21.10); A3 is owner-managed out of band.** As recorded at the time:
...
Every phase is done: Phase 0 (§21.1), Phase 1 (§21.2), Phase 2 (§21.3), Phase 3 (§21.4), Phase 4a (§21.6), Phase 4b (§21.7) and Phase 5 (§21.8), **and the series is closed — the closure ran 2026-07-27 (§21.10)**. §21.9 remains the audited inventory: Groups A (A1, A2), B and C struck with what discharged each, A3 owner-managed out of band, **Group D the live list of what stays deferred behind a stated gate**, and Group E the one open question. Nothing there is a phase; what is left needs a grill of its own, not execution.

The `Serializable` retry wrapper, `admin.rollOver`'s `list` and page, and `lot`'s `availableActions` were discharged by Phase 4b.

### 21.9 Closure inventory — audited 2026-07-27, executed the same day

> **Read this with §21.10.** Groups A (A1, A2), B and C are struck through with what discharged each; **A3 (migration baselining) and Group D are what is genuinely left**, and Group E is the one open question. The verified-state column is preserved as the audit found it, which is what makes the strike-throughs checkable.
...
Use this document as the primary architectural source. **The supplier-order commands (`src/server/services/admin/supplier-order.service.ts` + `operations-effects/supplier-order-effects.ts`) remain the behavioral bar for every new command**, now joined by `shipment.service.ts`'s `receive` — the richest quantity-moving command in the system, and the closest model for a Phase 4 delivery discrepancy — the operations-cart pattern plus ladder guards, in-transaction counter recompute, and server-computed `availableActions`; read them before writing a new command, together with `operation.service.ts`, which is the same pattern extended to a compound, all-or-nothing command, and `package.service.ts`'s `fractionate` — the only command that creates rows for several customers in one transaction and rolls a lot up afterwards. **Phases 0–5 are all built and the series is closed** (§21.10, 2026-07-27). **§21.9 remains the audited inventory**: Groups A (A1, A2), B and C are struck through with what discharged each, A3 is owner-managed out of band, **Group D is the live list — deferred by decision, each behind a stated gate**, and Group E is the one open question, re-armed with a concrete resolver rather than closed. Do not re-derive the owed list from the per-phase "Deliberately still owed" paragraphs: they are historical and §21.9 supersedes them. **Two commands are now the closure's guards**: `pnpm db:seed-verify` (stored-equals-derived, counters, zero criticals, per-enum coverage) and `pnpm fulfillment:e2e` (§21.7's twelve steps through the real service layer). Both need `--conditions=react-server`; run them after any change to derivation, diagnostics, the ladders or the seed. Two of §21.9's rows **corrected** earlier text — the seed carried two null-`deliveryMode` shipments rather than one (fixed, §21.10), and the migrations directory has no applied baseline at all (owner-managed, still open). §21.10 **corrects §21.8's method finding**: reaching the service layer from a script needs `--conditions=react-server`. §21.7 is the as-built record of 4b and §21.8 of 5; §21.5 and §21.6 remain the durable record of what the 4a/4b split settled — every structural item they owed is now discharged. **Every quantity-moving command must go through `runSerializable`**; `grep -rn "TransactionIsolationLevel.Serializable" src/` should return exactly one hit, inside the wrapper — **Phase 5 moved no quantity and did not add a thirteenth caller**. `carrier-order.service.ts` is the one command service that deliberately publishes **nothing** (§15 #10): do not "complete" it by wiring an effects handler while mirroring its siblings. §8's compensation reads as revised; the pre-revision wording survives there only as a marked counter-example. Read §4.1 sources before planning. Do not re-litigate §15 decisions unless repository evidence contradicts them; §20.1 is empty and §20.2 holds one non-blocking question, none of which gate anything now that every phase has landed. **What comes next is not a phase, and no longer series debt either.** The candidates are Group D's deferred items (each behind a stated gate), whatever §6.3 or §20.3 gets un-deferred, and the two findings the closure recorded without fixing — `listOriginalDemand` re-aggregating demand whose roll over was **resolved**, and the supplier MOQ being applied **per cart item rather than to pooled demand**. Both are *pre-execution* behaviour this document does not own (§6.4), and the second is a genuine domain question: pooling demand to reach a supplier minimum is arguably what an operation is for. Each wants a grill of its own; §22 routes them. Keep this document updated: bump the status header when a session revises it, and record invalidated assumptions rather than leaving them stale.
